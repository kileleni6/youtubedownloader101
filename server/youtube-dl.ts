...
export function formatThumbnails(thumbnails?: YouTubeDlThumbnail[]): { id: string; url: string; width: number; height: number; quality: string; ext: string }[] {
  if (!thumbnails) return [];
  
  // Common thumbnail resolutions we want to offer
  const desiredResolutions = [
    { width: 120, height: 90, quality: "lowest" },
    { width: 320, height: 180, quality: "low" },
    { width: 480, height: 360, quality: "medium" },
    { width: 640, height: 480, quality: "high" },
    { width: 1280, height: 720, quality: "hd" },
    { width: 1920, height: 1080, quality: "maxres" }
  ];
  
  const formattedThumbnails: { id: string; url: string; width: number; height: number; quality: string; ext: string }[] = [];
  
  // Sort thumbnails by width descending to get best quality first
  const sortedThumbnails = thumbnails.sort((a, b) => (b.width || 0) - (a.width || 0));
  
  // Find the best thumbnail for each desired resolution
  for (const desired of desiredResolutions) {
    // Find closest thumbnail that matches or is larger than desired resolution
    const closestThumbnail = sortedThumbnails.find(thumb => 
      (thumb.width || 0) >= desired.width && (thumb.height || 0) >= desired.height
    ) || sortedThumbnails[sortedThumbnails.length - 1]; // fallback to smallest available
    
    if (closestThumbnail) {
      const ext = closestThumbnail.url.includes('.webp') ? 'webp' : 
                  closestThumbnail.url.includes('.jpg') ? 'jpg' : 'png';
      
      formattedThumbnails.push({
        id: `thumb_${desired.quality}`,
        url: closestThumbnail.url,
        width: closestThumbnail.width || desired.width,
        height: closestThumbnail.height || desired.height,

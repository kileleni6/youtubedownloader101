...
import { getYouTubeVideoInfo, downloadYouTubeVideo, formatDuration, formatThumbnails } from "./youtube-dl";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/videos/info", async (req, res) => {
    try {
      ...
      const formattedInfo = {
        ...
        thumbnails: formatThumbnails(videoInfo.thumbnails)
      };
  ...
  // API route to download thumbnail
  app.post("/api/thumbnails/download", async (req, res) => {
    try {
      const { videoId, thumbnailId } = req.body;
      
      if (!videoId || !thumbnailId) {
        return res.status(400).json({ error: "Video ID and thumbnail ID are required" });
      }
      ...
      const thumbnail = videoInfo.thumbnails.find(t => t.id === thumbnailId);
      ...
      const filename = `${videoInfo.title.replace(/[^a-z0-9]/gi, '_')}_${thumbnail.quality}.${thumbnail.ext}`;
      
      // Set headers for file download
      res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
      res.setHeader("Content-Type", `image/${thumbnail.ext}`);
      ...
      res.status(500).json({ 
        error: error instanceof Error ? error.message : "Failed to download thumbnail" 
      });
    }
  });
